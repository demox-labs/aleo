...
impl ProvingKey {
    ...
    pub fn is_bond_public_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::BondPublicProver::METADATA)
    }
    ...
    pub fn is_claim_unbond_public_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::ClaimUnbondPublicProver::METADATA)
    }
    ...
    pub fn is_fee_private_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::FeePrivateProver::METADATA)
    }
    ...
    pub fn is_fee_public_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::FeePublicProver::METADATA)
    }
    ...
    pub fn is_inclusion_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::InclusionProver::METADATA)
    }
    ...
    pub fn is_join_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::JoinProver::METADATA)
    }
    ...
    pub fn is_set_validator_state_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::SetValidatorStateProver::METADATA)
    }
    ...
    pub fn is_split_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::SplitProver::METADATA)
    }
    ...
    pub fn is_transfer_private_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::TransferPrivateProver::METADATA)
    }
    ...
    pub fn is_transfer_private_to_public_prover(&self) -> bool {
        self.checksum()
            == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::TransferPrivateToPublicProver::METADATA)
    }
    ...
    pub fn is_transfer_public_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::TransferPublicProver::METADATA)
    }
    ...
    pub fn is_unbond_public_prover(&self) -> bool {
        self.checksum() == ProvingKey::prover_checksum(snarkvm_parameters::mainnet::UnbondPublicProver::METADATA)
    }
}
...
mod tests {
    ...
    fn get_proving_key_uri(function_name: &str, proving_key_metadata: &'static str) -> String {
        ...
        format!("https://mainnet.parameters.aleo.org/{}.prover.{}", function_name, checksum.get(0..7).unwrap())
    }

    #[wasm_bindgen_test]
    async fn test_proving_key_checksum() {
        let prover_uri = get_proving_key_uri("bond_public", snarkvm_parameters::mainnet::BondPublicProver::METADATA);
        ...
        let prover_uri =
            get_proving_key_uri("claim_unbond_public", snarkvm_parameters::mainnet::ClaimUnbondPublicProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_claim_unbond_public_prover());
        assert!(!prover.is_fee_private_prover());

        let prover_uri = get_proving_key_uri("fee_private", snarkvm_parameters::mainnet::FeePrivateProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_fee_private_prover());
        assert!(!prover.is_fee_public_prover());

        let prover_uri = get_proving_key_uri("fee_public", snarkvm_parameters::mainnet::FeePublicProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_fee_public_prover());
        assert!(!prover.is_join_prover());

        let prover_uri = get_proving_key_uri("join", snarkvm_parameters::mainnet::JoinProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_join_prover());
        ...
        let prover_uri =
            get_proving_key_uri("set_validator_state", snarkvm_parameters::mainnet::SetValidatorStateProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_set_validator_state_prover());
        assert!(!prover.is_split_prover());

        let prover_uri = get_proving_key_uri("split", snarkvm_parameters::mainnet::SplitProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_split_prover());
        ...
        let prover_uri =
            get_proving_key_uri("transfer_private", snarkvm_parameters::mainnet::TransferPrivateProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_transfer_private_prover());
        ...
        let prover_uri = get_proving_key_uri(
            ...
            snarkvm_parameters::mainnet::TransferPrivateToPublicProver::METADATA,
        );
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_transfer_private_to_public_prover());
        ...
        let prover_uri =
            get_proving_key_uri("transfer_public", snarkvm_parameters::mainnet::TransferPublicProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_transfer_public_prover());
        ...
        let prover_uri = get_proving_key_uri(
            ...
            snarkvm_parameters::mainnet::TransferPublicToPrivateProver::METADATA,
        );
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_transfer_public_to_private_prover());
        ...
        let prover_uri = get_proving_key_uri(
            ...
            snarkvm_parameters::mainnet::UnbondDelegatorAsValidatorProver::METADATA,
        );
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_unbond_delegator_as_validator_prover());
        ...
        let prover_uri =
            get_proving_key_uri("unbond_public", snarkvm_parameters::mainnet::UnbondPublicProver::METADATA);
        let proving_key_bytes = reqwest::get(prover_uri).await.unwrap().bytes().await.unwrap().to_vec();
        let prover = ProvingKey::from_bytes(&proving_key_bytes).unwrap();
        assert!(prover.is_unbond_public_prover());
        assert!(!prover.is_bond_public_prover());
